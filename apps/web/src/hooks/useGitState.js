...
import { checkoutBranch, commitChanges, createBranch, discardChanges, fetchChanges, getBranchStatus, getGitDiff, getGitLog, getGitRemotes, getGitRepos, getGitStatus, listBranches, pullChanges, pushChanges, stageFiles, unstageFiles, } from "../api";
export const createEmptyGitState = () => ({
    status: null,
    loading: false,
    error: null,
    diffPath: null,
 ...
 */
export const useGitState = (activeWorkspaceId, setStatusMessage) => {
    ...
    const updateGitState = useCallback((workspaceId, updater) => {
        setGitStates((prev) => ({
            ...prev,
    ...
    const refreshGitStatus = useCallback(async (targetWorkspaceId) => {
    ...
    const handleSelectRepo = useCallback(async (repoPath) => {
    ...
    const handleStageAll = useCallback(async () => {
        if (!activeWorkspaceId || !gitState.status)
            return;
        const repoPath = getCurrentRepoPath();
    ...
    const handleCommit = useCallback(async (message) => {
        if (!activeWorkspaceId || !message.trim())
            return;
        const repoPath = getCurrentRepoPath();
        try {
            const result = await commitChanges(activeWorkspaceId, message.trim(), repoPath);
            setStatusMessage(`Committed: ${result.summary.changes} changes, +${result.summary.insertions} -${result.summary.deletions}`);
            await refreshGitStatus();
        }
        catch (error) {
    ...
    const handleShowDiff = useCallback(async (file) => {
        if (!activeWorkspaceId)
            return;
        const repoPath = getCurrentRepoPath();
        updateGitState(activeWorkspaceId, (prev) => ({
    ...
    const handlePull = useCallback(async () => {
        ...
        try {
            ...
            if (result.summary.changes > 0) {
                setStatusMessage(`Pulled: ${result.summary.changes} changes, +${result.summary.insertions} -${result.summary.deletions}`);
            }
            else {
    ...
    const handleCreateBranch = useCallback(async (branchName, checkout = true) => {
        if (!activeWorkspaceId)
            return;
        const repoPath = getCurrentRepoPath();
        try {
    ...
    const getGitStateForWorkspace = useCallback((workspaceId) => {
