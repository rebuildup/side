...
import { TERMINAL_BACKGROUND_COLOR, TERMINAL_FONT_FAMILY, TERMINAL_FONT_SIZE, TERMINAL_FOREGROUND_COLOR, } from "../constants";
...
export function TerminalTile({ session, wsUrl, onDelete, onError }) {
    ...
    return (<div className="terminal-tile">
      <div className="terminal-tile-header" style={{ borderLeftColor: session.color || undefined }}>
        <div className="terminal-tile-header-content">
          {session.color && (<span className="terminal-color-indicator" style={{ backgroundColor: session.color }}/>)}
          <span className="terminal-tile-title">{session.title}</span>
          {session.tags && session.tags.length > 0 && (<div className="terminal-tile-tags">
              {session.tags.map((tag) => (<TerminalTag key={tag} tag={tag} color={session.color}/>))}
            </div>)}
        </div>
      </div>
