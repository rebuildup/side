...
export function EditorPane({ files, activeFileId, onSelectFile, onCloseFile, onChangeFile, onSaveFile, savingFileId, }) {
    ...
    const handleTabMiddleClick = (e, fileId) => {
        if (e.button === 1) {
            e.preventDefault();
            onCloseFile(fileId);
        }
    ...
    if (files.length === 0) {
        return (<div className="editor-container editor-empty">
    ...
    return (<div className="editor-container">
      ...
      <div className="editor-tabs">
        <div className="editor-tabs-list">
          {files.map((file) => {
            ...
            return (<div key={file.id} className={`editor-tab ${isActive ? "active" : ""} ${file.dirty ? "dirty" : ""}`} onClick={() => onSelectFile(file.id)} onMouseDown={(e) => handleTabMiddleClick(e, file.id)} role="tab" aria-selected={isActive} tabIndex={0}>
                <span className="editor-tab-icon" style={{ color }}>
                  {icon}
                </span>
                <span className="editor-tab-name">{file.name}</span>
                {file.dirty && !isSaving && (<span className="editor-tab-dirty" aria-label="未保存">
                    ●
                  </span>)}
                {isSaving && (<span className="editor-tab-saving" aria-label="保存中">
                    <Loader2 size={14} className="spin"/>
                  </span>)}
                <button type="button" className="editor-tab-close" onClick={(e) => handleCloseTab(e, file.id)} aria-label="閉じる">
                  <X size={14}/>
                </button>
              </div>);
        })}
      ...
      {/* Breadcrumb */}
      {activeFile && (<div className="editor-breadcrumb">
          <span className="editor-breadcrumb-path">{activeFile.path}</span>
        </div>)}

      {/* Editor Area */}
      <div className="editor-content">
        {activeFile ? (<Editor height="100%" theme={MONACO_THEME} language={activeFile.language} value={activeFile.contents} onChange={(value) => onChangeFile(activeFile.id, value ?? "")} onMount={handleEditorMount} options={{
