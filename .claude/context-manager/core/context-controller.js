...
class ContextController {
    ...
    constructor(store, monitor, analyzer) {
        this.store = store;
    ...
    async compact(options) {
        ...
        const { keepLastN = 20, preserveErrors = true, preserveSnapshots = true, dryRun = false, } = options || {};
        ...
        const eventsToCompact = session.events.filter(e => !uniqueEvents.some(kept => kept.timestamp === e.timestamp));
