...
class SnapshotManager {
   ...
   */
  async createSnapshot(sessionId, options = {}) {
   ...
   */
  getSnapshot(sessionId, commitHash) {
   ...
   */
  async restoreSnapshot(sessionId, commitHash) {
    const session = this.store.get(sessionId);
    if (!session) {
      throw new Error(`Session ${sessionId} not found`);
    }
   ...
   * Delete a snapshot
   */
  deleteSnapshot(sessionId, commitHash) {
    const session = this.store.get(sessionId);
    if (!session) {
      throw new Error(`Session ${sessionId} not found`);
    }
   ...
   */
  findSnapshotsByHealth(sessionId, minScore, maxScore) {
    const snapshots = this.getSnapshots(sessionId);
   ...
   */
  findSnapshotsByTime(sessionId, startTime, endTime) {
    const snapshots = this.getSnapshots(sessionId);
   ...
   */
  getHealthiestSnapshot(sessionId) {
    const snapshots = this.getSnapshots(sessionId);
    if (snapshots.length === 0) {
      return null;
    }
