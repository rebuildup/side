...
class OutputTrimmer {
   ...
   */
  trimToolResult(toolName: string, args: unknown, result: unknown): TrimmedOutput {
  ...
  private isNpmInstallLog(content: string): boolean {
    return /^(npm|yarn|pnpm|bun)(\s+install)?/m.test(content) ||
  ...
  private isTestResult(content: string): boolean {
    return /Tests:\s+\d+|passed|failed|skipped|PASS|FAIL/i.test(content) ||
  ...
  private trimNpmInstallLog(content: string): string {
    ...
    for (const line of lines) {
      ...
      if (match && match[1] && !match[1].startsWith('npm')) {
    ...
    const errors = lines.filter(line =>
      /error|ERR!|failed|E[A-Z]+/i.test(line) && !/deprecated/i.test(line)
    );
    if (errors.length > 0) {
      result.push('\n[Errors]');
  ...
  private trimJsonArray(content: string): string {
    try {
      ...
      result += sample.map((item, i) =>
        `${i + 1}. ${JSON.stringify(item, null, 2).split('\n').join('\n  ')}`
      ).join('\n\n');
  ...
  private trimSearchResults(content: string): string {
    ...
    const matchCount = lines.filter(line =>
      line.includes(':') && !line.startsWith('Binary file')
    ).length;
